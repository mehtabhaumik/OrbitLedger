...
Platform Admin now includes a dedicated report center for internal operational exports.

### Included

- Dedicated `Reports` section in Platform Admin.
- Report types:
  ...
  - Support Case Report.
- CSV generation for each report.
- Print-safe report view for each report.
- Report previews with all columns, horizontal scrolling where needed, and loaded-row count.
- Report metadata includes generated by, generated timestamp, row count, and active local filters.
- Quick action `Export report` now opens the report center instead of exporting an unrelated audit CSV.
- Mobile and tablet table previews keep all columns available through horizontal scrolling instead of hiding columns.
- The same horizontal-scroll table behavior now applies to workspace record tables and Platform Admin lists.

### Safety Boundaries

- Reports use the server-authorized Platform Admin snapshot and loaded audit trail.
- Reports do not expose passwords, payment provider secrets, API secrets, or customer ledger data.
- Report generation is client-side formatting of already-authorized admin data; it does not bypass server authorization.
- Browser pop-up failures show a branded inline error instead of a browser alert.

### Next Phase

`PLATFORM ADMIN PHASE 11: SaaS Safety Controls`
