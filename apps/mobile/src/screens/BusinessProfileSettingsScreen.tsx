...
export function BusinessProfileSettingsScreen({ navigation }: BusinessProfileSettingsProps) {
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      try {
        ...
        const workspaces = signedInCloudUser
          ? await listCloudWorkspacesForUser(signedInCloudUser.uid)
          : [];
  ...
  async function refreshCloudLifecycleState() {
    setIsLoadingCloudState(true);
    try {
      const signedInCloudUser = getCurrentCloudUser();
      setCloudUser(signedInCloudUser);
      if (!signedInCloudUser) {
        setCloudWorkspaces([]);
  ...
  async function handleSignOutFromCloud() {
    setIsLoadingCloudState(true);
    try {
  ...
  async function handleRunWorkspaceSync() {
    setIsRunningWorkspaceSync(true);
    try {
  ...
  async function enableSyncForCurrentBusiness() {
    if (!settingsSnapshot) {
      return;
    }
    ...
    const signedInCloudUser = getCurrentCloudUser();
    if (!signedInCloudUser) {
      navigation.navigate('CloudAuth', { returnTo: 'BusinessProfileSettings' });
      return;
    }

    setIsSyncWorkspaceActionBusy(true);
    try {
      const workspace = await createCloudWorkspace(
        signedInCloudUser,
        toWorkspaceProfileDraft(businessSettingsToFormValues(settingsSnapshot))
      );
      const saved = await saveBusinessSettings({
        ...
        taxMode: settingsSnapshot.taxMode,
        taxProfileVersion: settingsSnapshot.taxProfileVersion,
        taxProfileSource: settingsSnapshot.taxProfileSource,
        taxLastSyncedAt: settingsSnapshot.taxLastSyncedAt,
        taxSetupRequired: settingsSnapshot.taxSetupRequired,
      });
    ...
    try {
      ...
      if (saved.storageMode === 'synced' && saved.workspaceId && getCurrentCloudUser()) {
        ...
        await saveBusinessSettings({
          ...
          lastSyncedAt: updatedWorkspace.updatedAt,
          serverRevision: updatedWorkspace.serverRevision,
          taxMode: saved.taxMode,
          taxProfileVersion: saved.taxProfileVersion,
          taxProfileSource: saved.taxProfileSource,
          taxLastSyncedAt: saved.taxLastSyncedAt,
          taxSetupRequired: saved.taxSetupRequired,
        });
        await runWorkspaceSync();
        setSyncOverview(await getSyncOverview());
      }

      const refreshedSettings = await getBusinessSettings();
      if (!refreshedSettings) {
        throw new Error('Business profile could not be reloaded after saving.');
      }
      const savedFormValues = businessSettingsToFormValues(refreshedSettings);
      setSavedProfile(savedFormValues);
      setSettingsSnapshot(refreshedSettings);
      setCountryPackage(await loadCountryPackageForSettings(refreshedSettings));
    ...
    <SafeAreaView style={styles.root}>
      ...
      >
        ...
        >
          ...
          <Card
            elevated
            accent={settingsSnapshot?.storageMode === 'synced' ? 'premium' : 'primary'}
            ...
            {cloudUser ? (
              <View style={styles.cloudAccountBlock}>
                ...
                <View style={styles.syncActionRow}>
                  {settingsSnapshot?.storageMode !== 'synced' ? (
                    <PrimaryButton
                      loading={isSyncWorkspaceActionBusy}
                  ...
                  <PrimaryButton
                    loading={isRunningWorkspaceSync}
                  ...
                  <PrimaryButton
                    variant="ghost"
                    loading={isLoadingCloudState}
                    onPress={handleSignOutFromCloud}
                    style={styles.syncActionButton}
                  >
                    Sign Out
                  </PrimaryButton>
...
function toWorkspaceProfileDraft(values: ProfileFormValues): WorkspaceProfileDraft {
  return {
    businessName: values.businessName,
    ownerName: values.ownerName,
    phone: values.phone,
